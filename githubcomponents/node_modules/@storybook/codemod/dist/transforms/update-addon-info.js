...
function transformer(file, api) {
  ...
  const withInfo = addWithInfoExpression => {
    const {
      node
    } = addWithInfoExpression;
    const args = node.arguments; // if optional description string is not supplied, the story component becomes second arg
  ...
  const checkWithInfoImport = () => {
    const importExists = root.find(j.ImportDeclaration).filter(imp => imp.node.source.value === '@storybook/addon-info').size();
