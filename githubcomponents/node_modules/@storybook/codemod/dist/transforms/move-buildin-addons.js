...
function transformer(file, api) {
  ...
  const createImportDeclaration = (specifiers, source) => j.importDeclaration(specifiers.map(s => j.importSpecifier(j.identifier(s))), j.literal(source));

  const deprecates = {
    action: [['action'], '@storybook/addon-actions'],
    linkTo: [['linkTo'], '@storybook/addon-links']
  };
  const transform = j(file.source).find(j.ImportDeclaration).filter(i => i.value.source.value === '@storybook/react').forEach(i => {
